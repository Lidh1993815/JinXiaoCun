...
public class LuRecyclerView extends RecyclerView {
    ...
    private static final String TAG = "LuRecyclerView";
    private final RecyclerView.AdapterDataObserver mDataObserver = new DataObserver();//用来监听RecyclerView数据的变化
    private int mPageSize = 10; //一次网络请求默认数量

    private LuRecyclerViewAdapter mWrapAdapter;//对RecyclerViewAdapter进行的封装
    ...
    //判读adapter是否有数据，没有数据就显示数据为空时的占位图
    private class DataObserver extends RecyclerView.AdapterDataObserver {
        ...
        public void onItemRangeMoved(int fromPosition, int toPosition, int itemCount) {
            int headerViewsCountCount = mWrapAdapter.getHeaderViewsCount();//获得头部的数量
            //当数据变化时告诉观察者
    ...
    public void onScrolled(int dx, int dy) {
        ...
        RecyclerView.LayoutManager layoutManager = getLayoutManager();//获得recycler的布局
    ...
    private void calculateScrollUpOrDown(int firstVisibleItemPosition, int dy) {
        Log.i(TAG, "calculateScrollUpOrDown: "+firstVisibleItemPosition);
        if (null != mLScrollListener) {
            if (firstVisibleItemPosition == 0) {
                if (!mIsScrollDown) {
                    mIsScrollDown = true;
                    mLScrollListener.onScrollDown();//调用从上往下滑的监听
                }
            } else {
                //mDistance 滑动的距离
                //HIDE_THRESHOLD 触发上下滑动监听器的容差距离
                //mIsScrollDown 是否需要监听
                if (mDistance > HIDE_THRESHOLD && mIsScrollDown) {
                    mIsScrollDown = false;
                    mLScrollListener.onScrollUp();//从下往上滑，上拉
                    mDistance = 0;
                } else if (mDistance < -HIDE_THRESHOLD && !mIsScrollDown) {
                    mIsScrollDown = true;
                    mLScrollListener.onScrollDown();//下拉
                    mDistance = 0;
                }
            }
        }
