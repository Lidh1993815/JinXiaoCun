 ...
 * version: 1.0
 */
public class AttendanceButton extends View {
    ...
    public AttendanceButton(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AttendanceButton(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs, R.styleable.AttendanceButton, defStyleAttr, 0);
        ringBgColor = array.getColorStateList(R.styleable.AttendanceButton_ring_bg_color);
        ringColor = array.getColorStateList(R.styleable.AttendanceButton_ring_color);
        circleColor = array.getColorStateList(R.styleable.AttendanceButton_circle_color);
        textColor = array.getColorStateList(R.styleable.AttendanceButton_content_text_color);
        contentText = array.getString(R.styleable.AttendanceButton_content_text);
        mRadius = array.getDimensionPixelSize(R.styleable.AttendanceButton_radius, mRadius);
        textSize = array.getDimensionPixelSize(R.styleable.AttendanceButton_content_text_size, textSize);
        ringSize = array.getDimensionPixelSize(R.styleable.AttendanceButton_ring_size, 10);
        space = array.getDimensionPixelSize(R.styleable.AttendanceButton_space, 0);
        narrowDown = array.getDimensionPixelSize(R.styleable.AttendanceButton_narrow_down, 10);
        float dimension = context.getResources().getDimension(R.dimen._12sp);
        timeTextSize = array.getDimensionPixelSize(R.styleable.AttendanceButton_current_time_text_size, (int) dimension);
    ...
    private void initPaintOrPath() {
        ...
        timePaint.setColor(textColor.getColorForState(getDrawableState(), 0));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        String currentTime = DateUtil.getDate(System.currentTimeMillis(), "HH:mm:ss");
        canvas.drawText(currentTime, getWidth() / 2, getHeight() / 2+textSize, timePaint);
        postInvalidate();
    }
                ...
                angleAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator valueAnimator) {
                        ...
                        if (angle == 360) {
                            ...
                            onViewClick.onFinish(AttendanceButton.this);
