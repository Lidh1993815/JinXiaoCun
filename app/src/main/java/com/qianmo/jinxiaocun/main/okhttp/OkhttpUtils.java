...
public class OkhttpUtils {
    ...
    public static void sendRequest(int actionId, int requestMethod, String servlet, OkhttpParam okhttpParam, OnActionListener listener) {
        ...
        if (servlet.startsWith("http")) {
            okhttpCore = new OkhttpCore(actionId, servlet, MyApplication.application);//如果地址是以http开头，说明是完整的地址，也许是第三方的地址
        } else {
            okhttpCore = new OkhttpCore(actionId, OkhttpBase.BASE_URL + servlet, MyApplication.application);
        }
